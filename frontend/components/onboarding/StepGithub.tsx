"use client";

export function StepGithub() {
  return (
    <div>
      <h1 className="text-2xl font-semibold tracking-tight text-foreground">Connect your GitHub.</h1>
      <p className="mt-2 text-sm text-muted">
        Connect GitHub to verify your developer identity and unlock GitHub-powered Commit.ly features.
      </p>

      <div className="mt-8 rounded-xl border border-dashed border-border p-6 text-center">
        <button
          type="button"
          disabled
          title="GitHub connection is coming soon"
          className="focus-ring inline-flex cursor-not-allowed items-center gap-2 rounded-lg border border-border-strong px-5 py-2.5 text-sm font-medium text-muted opacity-60"
        >
          <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
            <path d="M12 .5C5.65.5.5 5.65.5 12c0 5.09 3.29 9.4 7.86 10.93.57.1.79-.25.79-.55 0-.27-.01-1.15-.02-2.09-3.2.7-3.88-1.35-3.88-1.35-.52-1.34-1.28-1.69-1.28-1.69-1.04-.72.08-.7.08-.7 1.16.08 1.77 1.19 1.77 1.19 1.03 1.77 2.7 1.26 3.36.96.1-.75.4-1.26.73-1.55-2.55-.29-5.24-1.28-5.24-5.7 0-1.26.45-2.29 1.19-3.09-.12-.29-.52-1.46.11-3.05 0 0 .97-.31 3.18 1.18a11.06 11.06 0 0 1 5.79 0c2.2-1.49 3.17-1.18 3.17-1.18.64 1.59.24 2.76.12 3.05.74.8 1.19 1.83 1.19 3.09 0 4.43-2.69 5.41-5.25 5.69.41.36.78 1.08.78 2.17 0 1.57-.01 2.83-.01 3.22 0 .3.21.66.8.55A10.53 10.53 0 0 0 23.5 12c0-6.35-5.15-11.5-11.5-11.5Z" />
          </svg>
          Connect GitHub
        </button>
        <p className="mt-3 text-xs text-muted-2">Coming soon — not yet available.</p>
      </div>
    </div>
  );
}
